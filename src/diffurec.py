...
class DiffuRec(nn.Module):
    ...
    def p_mean_variance(self, rep_item, item_rep1, x_t, t, mask_seq):
        model_output, _ = self.xstart_model(rep_item, x_t, self._scale_timesteps(t), mask_seq)
        model_output1, _ = self.xstart_model(item_rep1, x_t, self._scale_timesteps(t), mask_seq)
        model_output = (1 + self.w) * model_output - self.w * model_output1
    ...
    def p_sample(self, item_rep, item_rep1, noise_x_t, t, mask_seq):
        model_mean, model_log_variance = self.p_mean_variance(item_rep, item_rep1, noise_x_t, t, mask_seq)
    ...
    def reverse_p_sample(self, item_rep, item_rep1, noise_x_t, mask_seq):
        ...
        for i in indices: # from T to 0, reversion iteration  
            ...
            with th.no_grad():
                noise_x_t = self.p_sample(item_rep, item_rep1, noise_x_t, t, mask_seq)
        return noise_x_t 

    def forward(self, item_rep, item_tag, mask_seq):      
        noise = th.randn_like(item_tag).requires_grad_(True).to(self.device)
